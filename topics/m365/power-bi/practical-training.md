...
1. 1 페이지 이름 `Overview` 로 변경
2. 새 페이지 -> 2 페이지 이름 `Detail` 로 변경
3. Detail 페이지 -> 홈 탭 -> 삽입 -> 시각적 개체: `누적 세로 막대형 차트`
4. `누적 세로 막대형 차트` -> 데이터 패널 -> stores 쿼리 -> x축: `district`
...
### 3-6 다중 X축 드릴 탐색 (누적 세로 막대형 차트)

1. Overview 페이지 -> category 별 revenue 합계 누적 세로 막대형 차트 -> 데이터 패털: `subcategory` 추가
2. sub_category를 X축에 추가

### 3-7 날짜 타입 드릴 탐색 (꺽은선형 차트)

1. Overview 페이지 -> 홈 탭 -> 삽입 -> 시각적 개체: `꺽은선형 차트`
2. `꺽은선형 차트` 에서 데이터 패널 -> dates 쿼리 -> x축: `order_date`
3. 데이터 패널 -> sales 쿼리 -> Y축: `revenue`

### 3-8 보조 Y축과 계열 레이블 (꺽은선형 차트)

1. order_date 별 revenue로 꺽인선형 차트 -> Y축: `sales` 추가
2. Y축: `sales`를 보조 Y축에 옮기기
3. 서식 패널 -> 계열 레이블

### 3-9 확대/축소 슬라이더 (꺽은선형 차트)

1. order_date 별 revenue로 꺽인선형 차트 -> 서식 패널 -> 확대/축소 슬라이더
2. 확대/축소 슬라이더 -> Y축 끄기

### 3-10 Top 3 필터 (광역 구역 별 매출 Top 3)
...
1. 옵션 -> 보안 -> 사용자 지정 시각적 개체를 보고서에 추가할 때 보안 경고 표시, ArcGIS for Power BI 사용 체크 하기 (선택)
...
1. 홈 탭의 게시 -> `내 작업 영역`
2. 로그인
3. `https://powerbi.microsoft.com/` 접속
4. 로그인
