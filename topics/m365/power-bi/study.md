...
1. product 테이블에서 열추가 탭 -> 사용자 지정 열 클릭
2. 새 열 이름은 Volume 사용자 지정 열 수식은 `[length]*[depth]*[width]` 입력 후 확인
3. Volume 열을 10진수로 타입 변경

### 요일 자식 테이블 만들기 + 예제의 열 + 조건 열

1. sales 쿼리 우클릭 후 참조
2. date로 쿼리 이름 변경
3. 참조 테이블 의 order_date 헤드 우클릭 후 다른 열 제거로 해당 열만 남기기
4. order date 우클릭 후 중복된 항목 제거
5. 열 추가탭의 예제의 열 클릭
6. 추가된 더블클릭 후 요일 이름 선택 후 확인
7. 요일 이름 클릭 후 열 추가탭의 예제의 열(선택 항목에서) 클릭
8. 추가할 열에 Mon 또는 월 입력 후 열 이름 요일 약어 으로 변경 -> 확인
9. 열 추가 탭 -> 조건 열 -> 요일 이름의 값이 Monday 일 때 1을 출력 -> 반복, sunday는 else if 로 처리
10. sorting column 같은 제목으로 열 추가
...
1. sales와 date 쿼리의 order_date를 연결
